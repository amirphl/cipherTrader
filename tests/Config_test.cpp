...
TEST_F(ConfigTest, InitConfigSetsDefaults) {
  EXPECT_TRUE(std::get<bool>(
      Config::Config::getInstance().get("env.logging.order_submission")));
  EXPECT_EQ(std::get<int>(Config::Config::getInstance().get(
                "env.data.warmup_candles_num")),
            240);
  EXPECT_EQ(std::get<std::string>(
                Config::Config::getInstance().get("env.caching.driver")),
            "yaml");
}
...
TEST_F(ConfigTest, GetConfigNestedInvalidReturnsDefault) {
  auto value =
      Config::Config::getInstance().get("env.exchanges.NONEXISTENT.fee", 999);
...
TEST_F(ConfigTest, GetConfigEnvOverride) {
  ...
  auto value =
      Config::Config::getInstance().get("env.logging.order_submission", true);
...
TEST_F(ConfigTest, GetConfigCacheHit) {
  ...
  EXPECT_TRUE(Config::Config::getInstance().isCached(
      "env.logging.order_submission")); // Cached after first call
}
