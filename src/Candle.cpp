...
public:
  ...
  void reset() {
    ...
    close_price_ =
        RandomGenerator::getInstance().randint(0, 1)
    ...
    high_price_ = RandomGenerator::getInstance().randint(0, 1)
                      ? max_price
                      : std::max(max_price, max_price + 10);

    const int min_price = std::min(open_price_, close_price_);
    low_price_ = RandomGenerator::getInstance().randint(0, 1)
                     ? min_price
                     : std::min(min_price, min_price + 10);
  }
...
blaze::DynamicVector<T> fakeCandle(const blaze::DynamicVector<T> &attrs,
  ...
  candle[0] =
      (attrs[0] != T{}) ? attrs[0] : static_cast<T>(state.getTimestamp());
  candle[1] =
      (attrs[1] != T{}) ? attrs[1] : static_cast<T>(state.getOpenPrice());
  candle[2] =
      (attrs[2] != T{}) ? attrs[2] : static_cast<T>(state.getClosePrice());
  candle[3] =
      (attrs[3] != T{}) ? attrs[3] : static_cast<T>(state.getHighPrice());
  candle[4] =
      (attrs[4] != T{}) ? attrs[4] : static_cast<T>(state.getLowPrice());
...
template blaze::DynamicVector<double>
fakeCandle(const blaze::DynamicVector<double> &, bool);
template blaze::DynamicVector<float>
...
template blaze::DynamicMatrix<float>
