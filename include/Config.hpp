...
struct Conf
{
    ...
    struct App
    {
        ...
        std::vector< std::string > considering_candles;    // Assuming string for simplicity
        std::map< std::string, std::string > live_drivers; // Placeholder for live trade drivers
...
class Config
{
   public:
    ...
    ConfValue get(const std::string &keys, const ConfValue &defaultValue = std::string("")) const;
   ...
   private:
    ...
    ConfValue fetchValue(const std::string &keys, const ConfValue &defaultValue) const;
