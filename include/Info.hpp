...
// Convert unordered_map to string
template <typename T>
std::string mapToString(const std::unordered_map<T, bool> &map) {
  std::ostringstream oss;
  oss << "{";
  bool first = true;
...
inline std::string toString(const ExchangeInfo &var) {
  return std::visit(
...
inline std::vector<std::string> getExchangesByMode(const std::string &mode) {
